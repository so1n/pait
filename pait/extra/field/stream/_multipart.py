...
"""
Note:
    `python-multipart` uses the package name of `multipart`,
    it is not possible to import `multipart` via `import multipart`,
    so the `1.1.0` source code of `multipart` is introduced.
...
class PushMultipartParser:
    ...
    def parse(self, chunk: Union[bytes, bytearray]) -> Iterator[Union["MultipartSegment", bytearray, None]]:
        ...
        if self.content_length > -1 and self.content_length < self._parsed + len(self._buffer) + len(chunk):
...
class MultipartPart(object):
    ...
    def content_type(self) -> str:
        return self._segment.content_type or ("application/octet-stream" if self.filename else "text/plain")
