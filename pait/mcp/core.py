...
class BaseMCP(object):
    ...
    def _get_app_direct_dispatcher(self, app: Any) -> Callable:
        ...
        return build_direct_dispatcher(app, direct_dispatcher)
    ...
    def _get_mcp_route(self, app: Any) -> Callable:
...
class AsyncMCP(BaseMCP):
    ...
    def _get_mcp_route(self, app: Any) -> Callable:
        ...
        @pait()
        async def mcp_route(message: Dict[str, Any] = Json.i(default_factory=dict, raw_return=True)) -> Dict[str, Any]:
            return await self.handle_message(message)

        return mcp_route

    async def _call_tool_value(self, tool: MCPTool, arguments: Optional[Mapping[str, Any]]) -> Any:
...
class MCP(BaseMCP):
    ...
    def _get_mcp_route(self, app: Any) -> Callable:
        ...
        @pait()
        def mcp_route(message: Dict[str, Any] = Json.i(default_factory=dict, raw_return=True)) -> Dict[str, Any]:
            return self.handle_message(message)

        return mcp_route

    def _call_tool_value(self, tool: MCPTool, arguments: Optional[Mapping[str, Any]]) -> Any:
        """Execute a tool and return the raw dispatcher result."""
        core_model, real_call_mode = self._get_core_model_and_call_mode(tool)
        if real_call_mode == "http":
            if not self.app or not self.http_dispatcher:
