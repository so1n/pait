...
async def get_user_info(user_model: UserModel = Depends.i(depend.GetUserDepend)) -> JSONResponse:
...
class APIRouteCBV(HTTPEndpoint):
    content_type: str = Header.i(alias="Content-Type")

    @pait()
    async def get(self, user_model: UserModel = Depends.i(depend.GetUserDepend)) -> JSONResponse:
