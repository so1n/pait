...
async def async_like_multi_book_route(request_pydantic_model: social_p2p.LikeBookMapRequest) -> Any:
    ...
    loop: asyncio.AbstractEventLoop = asyncio.get_event_loop()
    if loop != stub.like_multi_book._loop:  # type: ignore
        raise RuntimeError(
            "Loop is not same, "
            "the grpc channel must be initialized after the event loop of the api server is initialized"
        )
    else:
        grpc_msg: Empty = await stub.like_multi_book(request_msg)
    return gateway.make_response(gateway.msg_to_dict(grpc_msg))
...
async def async_get_book_like_route(request_pydantic_model: social_p2p.GetBookLikesRequest) -> Any:
    ...
    loop: asyncio.AbstractEventLoop = asyncio.get_event_loop()
    if loop != stub.get_book_like._loop:  # type: ignore
        raise RuntimeError(
            "Loop is not same, "
            "the grpc channel must be initialized after the event loop of the api server is initialized"
        )
    else:
        grpc_msg: social_pb2.GetBookLikesListResult = await stub.get_book_like(request_msg)
    return gateway.make_response(gateway.msg_to_dict(grpc_msg))
...
async def async_comment_book_route(request_pydantic_model: social_p2p.CommentBookRequest) -> Any:
    ...
    loop: asyncio.AbstractEventLoop = asyncio.get_event_loop()
    if loop != stub.comment_book._loop:  # type: ignore
        raise RuntimeError(
            "Loop is not same, "
            "the grpc channel must be initialized after the event loop of the api server is initialized"
        )
    else:
        grpc_msg: Empty = await stub.comment_book(request_msg)
    return gateway.make_response(gateway.msg_to_dict(grpc_msg))
...
async def async_get_book_comment_route(request_pydantic_model: social_p2p.GetBookCommentRequest) -> Any:
    ...
    loop: asyncio.AbstractEventLoop = asyncio.get_event_loop()
    if loop != stub.get_book_comment._loop:  # type: ignore
        raise RuntimeError(
            "Loop is not same, "
            "the grpc channel must be initialized after the event loop of the api server is initialized"
        )
    else:
        grpc_msg: social_pb2.GetBookCommentListResult = await stub.get_book_comment(request_msg)
    return gateway.make_response(gateway.msg_to_dict(grpc_msg))
...
class StaticGrpcGatewayRoute(BaseStaticGrpcGatewayRoute):
    def gen_route(self) -> None:
        ...
        pait_async_like_book_route = like_book_route_pait()(async_like_book_route)
        pait_like_book_route = like_book_route_pait()(like_book_route)
        ...
        pait_async_like_multi_book_route = like_multi_book_route_pait()(async_like_multi_book_route)
        pait_like_multi_book_route = like_multi_book_route_pait()(like_multi_book_route)
        ...
        pait_async_get_book_like_route = get_book_like_route_pait()(async_get_book_like_route)
        pait_get_book_like_route = get_book_like_route_pait()(get_book_like_route)
        ...
        pait_async_get_book_like_route_1 = get_book_like_route_pait()(async_get_book_like_route)
        ...
        pait_get_book_like_route_1 = get_book_like_route_pait()(get_book_like_route)
        ...
        pait_async_comment_book_route = comment_book_route_pait()(async_comment_book_route)
        pait_comment_book_route = comment_book_route_pait()(comment_book_route)
        ...
        pait_async_get_book_comment_route = get_book_comment_route_pait()(async_get_book_comment_route)
        pait_get_book_comment_route = get_book_comment_route_pait()(get_book_comment_route)
