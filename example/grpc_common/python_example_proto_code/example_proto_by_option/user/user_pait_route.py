...
async def async_logout_user_route(
    ...
    grpc_msg: Empty = await stub.logout_user(request_msg, metadata=[("req_id", req_id)])
...
def logout_user_route(
    ...
    request_dict["token"] = token
    request_msg: user_pb2.LogoutUserRequest = gateway.get_msg_from_dict(user_p2p.LogoutUserRequest, request_dict)
    grpc_msg: Empty = stub.logout_user(request_msg, metadata=[("req_id", req_id)])
    return gateway.make_response(gateway.msg_to_dict(grpc_msg))
...
async def async_login_user_route(request_pydantic_model: user_p2p.LoginUserRequest) -> Any:
    ...
    if loop != stub.login_user._loop:  # type: ignore
        raise RuntimeError(
            "Loop is not same, "
            "the grpc channel must be initialized after the event loop of the api server is initialized"
        )
    else:
        grpc_msg: user_pb2.LoginUserResult = await stub.login_user(request_msg)
    return gateway.make_response(gateway.msg_to_dict(grpc_msg))
...
async def async_create_user_route(request_pydantic_model: user_p2p.CreateUserRequest) -> Any:
    ...
    loop: asyncio.AbstractEventLoop = asyncio.get_event_loop()
    if loop != stub.create_user._loop:  # type: ignore
        raise RuntimeError(
            "Loop is not same, "
            "the grpc channel must be initialized after the event loop of the api server is initialized"
        )
    else:
        grpc_msg: Empty = await stub.create_user(request_msg)
    return gateway.make_response(gateway.msg_to_dict(grpc_msg))
...
class StaticGrpcGatewayRoute(BaseStaticGrpcGatewayRoute):
    def gen_route(self) -> None:
        ...
        pait_async_logout_user_route = logout_user_route_pait()(async_logout_user_route)
        pait_logout_user_route = logout_user_route_pait()(logout_user_route)
        ...
        pait_async_login_user_route = login_user_route_pait()(async_login_user_route)
        pait_login_user_route = login_user_route_pait()(login_user_route)
        ...
        pait_async_create_user_route = create_user_route_pait()(async_create_user_route)
        pait_create_user_route = create_user_route_pait()(create_user_route)
        ...
        pait_async_delete_user_route = delete_user_route_pait()(async_delete_user_route)
        pait_delete_user_route = delete_user_route_pait()(delete_user_route)
