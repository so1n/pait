...
class APIRouteCBV(HTTPMethodView):
    content_type: str = Header.i(alias="Content-Type")

    @pait()
    async def get(self, user_model: UserModel = Depends.i(depend.GetUserDepend)) -> response.HTTPResponse:
        return response.json({"code": 0, "msg": "ok", "data": user_model.dict(), "content_type": self.content_type})
