...
class TestPaitModel(BaseModel):
    uid: conint(gt=10, lt=1000) = FactoryField(Query())
    user_name: constr(min_length=2, max_length=4) = FactoryField(Query())
    user_agent: str = FactoryField(Header(key='user-agent'))
